...
d3.json("ferrariData.json").then(function (ferrariData) {
  ...
  function drawViz(dataset) {
    ...
    window.addEventListener(
      ...
      function (event) {
        ...
        for (let i = 1; i < ferrariData.length; i++) {
        ...
        if (raceInFocus) {
          let inFocusNode = d3
            .select(`#r${raceInFocus.raceIdFerrari + numPerRow}`)
            .node();
        ...
        currentStoryLinks.on("click", function (d) {
          ...
          setTimeout(function () {
            d3.select(`#r${raceInFocus.raceIdFerrari}`)
              .node()
  ...
  function tooltipFunction(raceObj) {
    ...
    d3.select("#races-count").text(
      "Race #" + raceObj.raceIdFerrari + " of " + ferrariData.length
    );
